...
namespace Bitzy.Buffs
{
	public class GlitzyBuff : ModBuff
	{
		public override void SetDefaults() {
			...
			Description.SetDefault("How can he be so harmless?");
			...
			if (player.controlDown && player.releaseDown) {
				if (player.doubleTapCardinalTimer[0] > 0 && player.doubleTapCardinalTimer[0] != 15) {
					for (int j = 0; j < 1000; j++) {
						if (Main.projectile[j].active && Main.projectile[j].type == ProjectileType<Projectiles.Pets.GlitzyProj>() && Main.projectile[j].owner == player.whoAmI) {
