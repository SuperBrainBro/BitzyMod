...
namespace Bitzy.Items.Weapons
{
	public class BitzySword : ModItem
	{
		public override void SetStaticDefaults() {
			Tooltip.SetDefault("Hits heal for health");  //The (English) text shown below your weapon's name
		}
		...
		public override void SetDefaults() {
			...
			item.useTime = 24; // The time span of using the weapon. Remember in terraria, 60 frames is a second.
			item.useAnimation = 24; // The time span of the using animation of the weapon, suggest setting it the same as useTime.
			item.knockBack = 8; // The force of knockback of the weapon. Maximum is 20
			item.value = Item.buyPrice(gold: 1); // The value of the weapon in copper coins
			item.rare = 3; // The rarity of the weapon, from -1 to 13. You can also use ItemRarityID.TheColorRarity
		...
		public override void OnHitNPC(Player player, NPC target, int damage, float knockback, bool crit) {
			...
			// 60 frames = 1 second
			player.HealEffect(damage / 25, true);
		}
